...
app.childernHealth = kendo.observable({
    onShow: function() {},
    afterShow: function() {}
});

// START_CUSTOM_CODE_childernHealth
function addToCart(e) {
    ...
    app.insertRecordUpdateRecord = function(ProductName, Catalogid, size, Image_URL, Catalogy) {
                ...
                tx.executeSql("SELECT * FROM CartTable where Catalogid = ? ", [Catalogid],
                    function(tx, rs) {
                        ...
                        if (rs.rows.length < 1) {
                            ...
                            app.db.transaction(function(tx) {
                                tx.executeSql("INSERT INTO CartTable(ProductName, Catalogid ,size ,Image_URL ,Catalogy,count ) VALUES (?,?,?,?,?,?)", [ProductName, Catalogid, size, Image_URL, Catalogy, 1],
                                    app.onSuccess,
                                    app.onError);
                            });
                        } else {
                            app.db.transaction(function(tx) {
                                tx.executeSql("UPDATE CartTable SET count = count + 1 WHERE Catalogid = ?", [Catalogid],
                                    app.onSuccess,
                                    app.onError);
                            });
            ...
            app.db.transaction(function(tx) {
                var cDate = new Date();
                tx.executeSql("INSERT INTO CartTable(ProductName, Catalogid ,size ,Image_URL ,Catalogy ) VALUES (?,?,?,?,?)", [ProductName, Catalogid, size, Image_URL, Catalogy],
                    app.onSuccess,
                    app.onError);
            });
        }
    ...
    app.deleteRecord = function(id) {
            app.db.transaction(function(tx) {
                tx.executeSql("DELETE FROM CartTable WHERE Catalogid = ?", [id],
                    app.onSuccess,
                    app.onError);
            });
        }
...
// END_CUSTOM_CODE_childernHealth
...
(function(parent) {
    var dataProvider = app.data.himalayaPlaygroundBackend,
        ...
        childernHealthModel = kendo.observable({
            dataSource: dataSource,
            itemClick: function(e) {
                app.mobileApp.navigate('#components/childernHealth/details.html?uid=' + e.dataItem.uid);
            },
            detailsShow: function(e) {
                var item = e.view.params.uid,
                    dataSource = childernHealthModel.get('dataSource'),
                ...
                childernHealthModel.set('currentItem', itemModel);
            },
            currentItem: null
        });

    parent.set('childernHealthModel', childernHealthModel);
})(app.childernHealth);

// START_CUSTOM_CODE_childernHealthModel
// END_CUSTOM_CODE_childernHealthModel